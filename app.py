...
@login_required
def inbox():
    """Show All emails Recived"""
    userId = session.get("user_id") # get user_id from session
    user = db.execute("SELECT * FROM users WHERE id = ?", userId) # get user from users table using user_id
    username = user[0]["username"]
    emails = db.execute("SELECT * FROM emails WHERE recipient = ?", username) # get emails with the user's username as recipient
...
def sent():
    """Show send emails"""
    userId = session ["user_id"]
    usernameDB = db.execute("SELECT username FROM users WHERE id = ?", userId) # get the username of the current user
    username = usernameDB[0]["username"]
    emails = db.execute("SELECT * FROM emails WHERE sender = ?", username) # get emails with the user's username as sender
    return render_template("index.html", emails=emails)
...
def register():
    ...
    if request.method == "GET":
        ...
        # Remember user_id and redirect to homepage
